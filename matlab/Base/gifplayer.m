function handles = gifplayer(gif_image,delay_length)
...
if strcmp(ext,'.gif')
    ...
    handles.h1 = imshow(handles.im(:,:,:,1),handles.map,'Border','tight');%loads the first image along with its colormap
    handles.guifig = gcf;
    
    %% added by alex
    handles.guifig.Name = 'Loading MyoDashy';
    handles.guifig.NumberTitle = 'off';
    handles.guifig.MenuBar = 'none';
    handles.guifig.ToolBar = 'none'; 
    handles.guifig.Units = 'normalized'; 
    handles.guifig.OuterPosition = [0.30,0.27,0.38,0.53];
